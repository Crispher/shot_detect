...
__min_duration__ = 1           # if a shot has length less than this, merge it with others
__absolute_threshold__ = 0.8   # any transition must be no less than this threshold
...
class shotDetector: 
    ...
    def run(self, video_path=None):
        ...
        while True:
            ...
            color_histgrams = [cv2.calcHist([frame], [c], None, [__hist_size__], [0,256]) \
                          for c in range(3)]
        ...
        # scores = [cv2.compareHist(pair[0],pair[1],method=cv2.HISTCMP_CHISQR) \
